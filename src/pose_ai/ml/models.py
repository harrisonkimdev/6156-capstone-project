"""BiLSTM and Transformer multitask models for efficiency regression and next-action classification."""
...
class TransformerConfig:
    """Configuration for Transformer multitask model."""
    input_dim: int = 60  # Feature dimension
    d_model: int = 128  # Model dimension (must be divisible by nhead)
    nhead: int = 8  # Number of attention heads
    num_layers: int = 4  # Number of transformer layers
    dim_feedforward: int = 512  # Feedforward network dimension
    dropout: float = 0.3  # Dropout rate
    num_action_classes: int = 5  # Number of next-action classes (0-4)
    pooling: str = "mean"  # Pooling strategy: "mean", "max", or "cls"
    positional_encoding: str = "sinusoidal"  # "sinusoidal" or "learnable"


class PositionalEncoding(nn.Module):
    """Positional encoding for transformer sequences.
    
    Adds temporal information to input features using either sinusoidal
    encoding (fixed) or learnable position embeddings.
    """
...
class TransformerMultitaskModel(nn.Module):
    """Transformer model with multitask heads for efficiency regression and next-action classification."""
    ...
    def __init__(self, config: Optional[TransformerConfig] = None):
        ...
        self.pos_encoder = PositionalEncoding(
            ...
            max_len=100,
            encoding_type=self.config.positional_encoding,
            dropout=self.config.dropout,
        )
    ...
    def forward(
        ...
        x = self.pos_encoder(x)  # (batch, seq_len[+1], d_model)
        ...
        if self.config.pooling == "cls":
            ...
            # Mean pooling
            if self.cls_token is not None:
                pooled = x[:, 1:, :].mean(dim=1)  # Exclude CLS token
            else:
                pooled = x.mean(dim=1)  # (batch, d_model)
        elif self.config.pooling == "max":
            # Max pooling
            if self.cls_token is not None:
                pooled = x[:, 1:, :].max(dim=1)[0]  # Exclude CLS token
            else:
    ...
    @classmethod
    def load(cls, path: Path, device: str = "cpu") -> "TransformerMultitaskModel":
        """Load model from checkpoint."""
        if not HAS_TORCH:
            raise ImportError("PyTorch is required. Install with: pip install torch")
